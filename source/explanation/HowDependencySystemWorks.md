...
![[Pasted image 20260703141822.png]]
...
![[Pasted image 20260703142200.png]]

And in this case when `total_cost_per_part` is calculated it will first recurse and calculate `post_processing_cost_per_part`. Then calculate all others in "depends_on" list. Then finally calculate `total_cost_per_part`. 
...
![[Pasted image 20260703143716.png]]

You can see in this example that both `total_cost` and `direct_cost_per_part_excluding_labor` both depend on `material_cost_per_part`. Also notice that `total_cost` indirectly depends on `direct_cost_per_part_excluding_labor`.
